import v8n from "v8n";
import { validFinanceTypes, type TFinance } from "./index";
...
function validateType(type: any) {
  const errors: string[] = [];
  if (!validFinanceTypes.includes(type)) {
    errors.push(
      "In valid type : it must be one off =>  " + validFinanceTypes.join(", ")
    );
  }
  return errors;
}

export function validateFinance(finance: TFinance) {
  const errors = {
