...
import { storeAllowance } from "@/app/modules/salary/allowances";
...
const allowance: Ref<AllowanceOptionsRecord> = ref({
  title: undefined,
  amount: undefined,
...
async function save() {
  ...
  loading.start();
  await storeAllowance(allowance.value);
  loading.end();
  ...
  $activeModalsBus.$emit('allowance:created')
  emit('saved')
}
