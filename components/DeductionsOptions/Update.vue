...
import { storeDeduction, updateDeduction } from "@/app/modules/salary/deductions";
...
const deduction: Ref<DeductionOptionsRecord> = defineModel();
...
async function save() {
  ...
  loading.start();
  await updateDeduction(deduction.value);
  loading.end();
  ...
  $activeModalsBus.$emit('deduction:updated')
  emit('saved')
}
