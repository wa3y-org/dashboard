...
const donationsList = ref<TFinance[]>([])
...
onMounted(() => {
  loadDonations();
  useNuxtApp().$activeModalsBus.$on('finance:created', loadDonations)
  useNuxtApp().$activeModalsBus.$on('finance:updated', loadDonations)
  useNuxtApp().$activeModalsBus.$on('finance:removed', loadDonations)
