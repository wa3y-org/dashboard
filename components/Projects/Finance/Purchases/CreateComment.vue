...
async function save() {
    ...
    loading.start();
    const response = await useProjectPurchasesComityComments().create(post.value, props.request?.id || '');
    loading.end();
    ...
    if (response.model) {
        ...
        useNuxtApp().$activeModalsBus.$emit('projects:purchases:comments:created')
    }
}
</script>
