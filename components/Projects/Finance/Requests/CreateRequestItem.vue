    ...
    <v-card rounded="xl" :loading="loading.isLoading.value" :disabled="loading.isLoading.value">
      ...
      <v-card-text>
        ...
        <v-row>
          ...
          <v-col>
            <NumberField :model-value="totalPrice" name="Total Price" placeholder=""
              :props="{
                readonly: true,
                suffix: ' ' + itemData.currency || '',
              }" />
        ...
        <v-row>
          ...
          <v-col>
            <NumberField :model-value="totalUSD" name="Total USD" placeholder=""
              :props="{
                readonly: true,
                suffix: ' USD',
              }" />
...
async function save() {
  ...
  const response = await useFinancialRequestsItems().create(itemDataClone, props.request.id);
  loading.end();
  ...
  if (response.model) {
    ...
    useNuxtApp().$activeModalsBus.$emit('projects:financial_requests:items:created')
    emit('saved')
  }
}
