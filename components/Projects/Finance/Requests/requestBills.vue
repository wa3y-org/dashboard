<template>
    ...
    <div class="pa-4">
        ...
        <div v-for="(bill, index) in bills" :key="index + bill">
            <p variant="text" color="blue" class="font-weight-thin text-caption text-blue cursor-pointer" link
...
async function confirmRemove(bill: string) {
    ...
    if (isConfirmed) {
        await Remove.doRemove(async () => {
            // useFinancialRequestsItems().bills.remove(props.request, bill);
            const response = await useFinancialRequests().update(Object.assign({ id: props.request?.id, 'bills-': bill },));

        });
        useNuxtApp().$activeModalsBus.$emit('projects:financial_requests:updated');
...
async function uploadBills() {
    ...
    backendError.clear();
    loading.start();
    const response = await useFinancialRequests().update(Object.assign({ id: props.request?.id, 'bills+': billsToUpload.value },));
    loading.end();
