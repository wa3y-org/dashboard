        ...
        <v-expansion-panel value="createPostMainPanel" :loading="loading.isLoading.value"
          ...
          <v-expansion-panel-text>
            <div class="py-4">
              ...
              <p class="my-4"></p>
              <v-checkbox v-model="member.can_create" hide-details color="success"
                label="Can Create Financial Request" />
              <v-checkbox v-model="member.can_update_status" hide-details color="success"
                label="Can Update Financial Request's Status" />
            </div>
          </v-expansion-panel-text>
...
const validationErrors = computed(() => {
  ...
  return useProjectFinancailCommity().validate(member.value);
});
...
async function save() {
  ...
  const response = await useProjectFinancailCommity().create(memberClone, props.project.id || '');
  loading.end();
