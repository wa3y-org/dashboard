...
const validationErrors = computed<{ [key: string]: any }>(() => {
  ...
  return ActivityFinance.validate(props.finance.project, financeData.value)
});
...
async function save() {
  ...
  loading.start();
  const response = await ActivityFinance.update(Object.assign({ id: props.finance.id }, financeData.value));
  loading.end();
  ...
  if (response.model) {
    ...
    useNuxtApp().$activeModalsBus.$emit('activities:finance:updated')
    emit('saved')
  }
}
</script>
