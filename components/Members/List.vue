<template>
  <div>
    ...
    <v-data-table :loading="membersFetcher.loading.isLoading.value" :headers="headers"
      :items="membersFetcher.membersList.value">
      <template v-slot:item.avatar="{ item }">
        <NuxtLink :to="`/hr/members/${item.id}`">
          <div class="d-flex align-center">
            <v-avatar cover size="84" rounded="lg" class="my-2 elevation-1">

              <v-img :src="getAvatarUrl(item)" alt="member avatar image" />
