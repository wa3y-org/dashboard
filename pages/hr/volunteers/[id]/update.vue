    ...
    <v-stepper v-model="step" show-actions editable>
      ...
      <v-stepper-window style="min-height: 24rem;" class="pa-0 ma-0 my-4">
        <v-stepper-window-item :value="1">
          <create-volunteer-personal-info v-model="volunteer" />
        </v-stepper-window-item>
        <!-- <v-stepper-window-item :value="2">
          <w-id-info />
        </v-stepper-window-item> -->
        <v-stepper-window-item :value="2">
          <create-volunteer-contact-info v-model="volunteer" />
        </v-stepper-window-item>
        <v-stepper-window-item :value="3">
          <create-volunteer-membership-info v-model="volunteer" />
        </v-stepper-window-item>
        <v-stepper-window-item :value="4">
          <create-volunteer-salary v-model="volunteer" />
        </v-stepper-window-item>
      </v-stepper-window>
...
import { validateVolunteer } from "~/app/modules/volunteers/validateVolunteer";
import { updateVolunteer } from "~/app/modules/volunteers/update";
const step = ref(1);
const items = [
...
const volunteer: Ref<VolunteersRecord | any> = ref({
  basic_salary: 0,
  name: '',
  address: '',
  allowances: [],
...
const { loadOneVolunteer } = useVolunteers();
...
const validationErrors = computed(() => {
  return validateVolunteer(volunteer.value);
});
...
async function save() {
  ...
  loading.start()
  const record = await updateVolunteer(volunteer.value);
  loading.end();

  useRouter().push(`/hr/volunteers/${record.id}`)
}
